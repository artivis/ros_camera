              ...
              int rad_flag)
{
  ...
  {
    sph_coor = cv::Mat::ones(2, cart_coor.cols, CV_32F);
...
{
  cv::Mat cart_pts_tmp = cv::Mat::zeros(3, sph_pts.cols, CV_32F);
  ...
  cart_pts_tmp.convertTo(cart_pts, CV_32F);
}
...
void sph2Heal(const cv::Mat &sph_pts, cv::Mat &heal_pts)
{
  ...
  heal_pts = cv::Mat::zeros(2, sph_pts.cols, CV_32F);
  ...
  for (int i=0; i<sph_pts.cols; ++i)
  {
    ...
    if (sph_pts.at<float>(1,i) < L_PI)
      azi = sph_pts.at<float>(1,i);
    else
      azi = sph_pts.at<float>(1,i) - (2*L_PI);

    if( std::abs(el) < theta_c )
    {
      heal_pts.at<float>(1,i) = azi;

      heal_pts.at<float>(0,i) = ((K*L_PI)/(2.*H)) * std::sin(el);
    }
    else if ( el < 0. )
    {
      xi_c = std::sqrt( K * ( 1. - std::abs( std::sin(el) ) ) );

      phi_c = -L_PI + ( 2. * std::floor( ((azi+L_PI)*H)/(2.*L_PI) ) + 1. ) * (L_PI / H);

      heal_pts.at<float>(1,i) = phi_c + (azi-phi_c) * xi_c;

      heal_pts.at<float>(0,i) = - (L_PI/H) * (2.-xi_c);
    }
    else
    {
      xi_c = std::sqrt( K * ( 1. - std::abs( std::sin(el) ) ) );
...
void rotateCloudPoint(cv::Mat &ClPts, float roll,
                      ...
                      bool rad)
{
  ClPts = getRotationMat(roll, pitch, yaw, rad) * ClPts;
}
